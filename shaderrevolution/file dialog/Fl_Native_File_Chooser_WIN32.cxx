...
void FNFC_CLASS::ClearOFN() {
    ...
    if ( _ofn.lpstrFile ) {
        _ofn.lpstrFile = (LPWSTR)strfree((char*)_ofn.lpstrFile);
    }
    if ( _ofn.lpstrInitialDir ) {
        _ofn.lpstrInitialDir = (LPCWSTR)strfree((char*)_ofn.lpstrInitialDir);
    }
...
int FNFC_CLASS::showfile() {
    ...
    _ofn.lpstrFile    = (LPWSTR)new char[fsize];
    ...
    // DIALOG TITLE
    _ofn.lpstrTitle = (LPCWSTR)_title ? (LPCWSTR)_title : NULL;
    // FILTER
    _ofn.lpstrFilter = (LPCWSTR)_parsedfilt ? (LPCWSTR)_parsedfilt : NULL;
    // PRESET FILE
	...
	strncpy((char*)_ofn.lpstrFile, _preset_file, _ofn.nMaxFile);
	Unix2Win((char*)_ofn.lpstrFile);
	...
	//
	_ofn.lpstrInitialDir = (LPCWSTR)strnew(_directory);
	...
	err = GetSaveFileName((LPOPENFILENAMEA)&_ofn);
    } else {
	err = GetOpenFileName((LPOPENFILENAMEA)&_ofn);
    }
    if ( err == 0 ) {
	...
	case BROWSE_FILE: 
	case BROWSE_SAVE_FILE:
	    set_single_pathname((LPCSTR)_ofn.lpstrFile);
	    Win2Unix(_pathnames[_tpathnames-1]);
	    break;
	case BROWSE_MULTI_FILE: {
	    // EXTRACT MULTIPLE FILENAMES
	    const char *dirname = (LPCSTR)_ofn.lpstrFile;
		...
		//
		char pathname[2048]; 
		for ( const char *s = (LPCSTR)_ofn.lpstrFile + dirlen + 1; 
						      *s; s+= (strlen(s)+1)) {
...
int FNFC_CLASS::showdir() {
    ...
    // DIALOG TITLE
    _binf.lpszTitle = (LPCWSTR)_title ? (LPCWSTR)_title : NULL;
    // FLAGS
    ...
    _binf.pszDisplayName = (LPWSTR)displayname;
    // PRESET DIR
    ...
    _binf.lpfn = Dir_CB;
    // OPEN BROWSER
    ITEMIDLIST *pidl = SHBrowseForFolder((LPBROWSEINFOA)&_binf);
    // CANCEL?
