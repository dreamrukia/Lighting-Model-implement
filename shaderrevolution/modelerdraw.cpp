...
float nextT(float t, Point2d p0, Point2d p1) {
	float result = pow((p1.x - p0.x), 2) + pow(p1.y - p0.y, 2.0f);
	result = pow(result, .5f);
	result = pow(result, .5f);
	return result + t;
}
...
void drawMyObject(double scale){
	...
	Point2d b1, b2, c;
	float points = 100;
	for (int i = 0; i + 3 < revolution_pts.size(); i++) {
		p0 = revolution_pts[i];
		p1 = revolution_pts[i + 1];
		p2 = revolution_pts[i + 2];
		p3 = revolution_pts[i + 3];
		float t0 = 0;
		float t1 = nextT(t0, p0, p1);
		float t2 = nextT(t1, p1, p2);
		float t3 = nextT(t2, p2, p3);
		for (float t = t1; t < t2; t += (t2 - t1) / points) {
...
/***********************************************************
Your code will interact with the drawRevolution routine below.
Strategy: 
i. Currently points in revolution_pts are rotated around the z axis to generate a 3D figure
ii. Rather than the revolution_pts, you would like points produced from an interpolated spline (Catmull-Rom)
iii. There is already a variable new_pts declared, that can be used to insert your new points
...
void drawRevolution(double scale)
{
	...
	} else    
	{
		...
		//}
//*****************************************
// The code below is the same as above, but spline_pts and new_pts are used in place of num_pts and revoltion_pts
// You can swap this code for the section above when you're ready

for ( int j = 1; j < new_pts.size()-1;  j ++) {
			 //for each point, make corresponding sets of points
			for (int lgt = 0; lgt < divisions;lgt++){
				float theta = 2*PI*lgt/divisions;
				Vec3d curr = Vec3d(new_pts[j].x * cos(theta),new_pts[j].y+origin_y,
					(new_pts[j].x * sin(theta))+origin_z);
				Vec3d curr_low =  Vec3d(new_pts[j-1].x * cos(theta),new_pts[j-1].y+origin_y,
					(new_pts[j-1].x * sin(theta))+origin_z);
				int lgtNext = (lgt==divisions?0:lgt+1);
				float thetaNext= 2*PI*lgtNext/divisions;
				Vec3d curr_nxt= Vec3d(new_pts[j].x * cos(thetaNext), new_pts[j].y+origin_y,
					(new_pts[j].x * sin(thetaNext))+origin_z);
				Vec3d curr_low_nxt = Vec3d( new_pts[j-1].x * cos(thetaNext),
					new_pts[j-1].y+origin_y, (new_pts[j-1].x * sin(thetaNext))+origin_z);
				//compute normals
				Vec3d curr_low_n =(cp(Vec3d(-1*sin(theta),0,cos(theta)),curr-curr_low));
				Vec3d curr_n = (cp(Vec3d(-1*sin(theta),0,cos(theta)),curr-curr_low));
				Vec3d curr_nxt_n = (cp(Vec3d(-1*sin(thetaNext),0,cos(thetaNext)),curr_nxt-curr_low_nxt));
