...
ModelerUserInterface::ModelerUserInterface() {
	...
	Fl_Double_Window* w;
	const char* title = "A Shader Revolution";
	{ Fl_Double_Window* oWin = m_controlsWindow = new Fl_Double_Window(1024, 768, title);
		w = oWin;
		oWin->callback((Fl_Callback*)cb_m_controlsWindow, (void*)(this));
		oWin->when(FL_WHEN_NEVER);
		{ Fl_Menu_Bar* oMen = m_controlsMenuBar = new Fl_Menu_Bar(0, 0, 1024, 25);
		  oMen->menu(menu_m_controlsMenuBar);
		}

		// Contains the controls on the left
		{ leftPane = new Fl_Group(0, 25, 200, 768-25);
			int tabSpace = 25, controlSpace = 0;
				{ Fl_Tile* oCont = m_controlSplitPane = new Fl_Tile(0, 25, 200, 768-25);
					oCont->box(FL_FLAT_BOX);
					{ Fl_Tree* oTree = m_controlsTree = new Fl_Tree(0, 25, 200, 125);
					  oTree->when(FL_WHEN_CHANGED);
					  oTree->callback((Fl_Callback*)TreeCallback);
					  oTree->marginleft(-5); //-5
					  oTree->end();
					} 
					{ Fl_Scroll* oScroll = m_controlsScroll =
						new Fl_Scroll(0, 125, 200, 768-125);
					  oScroll->type(Fl_Scroll::VERTICAL);
					  oScroll->when(FL_WHEN_CHANGED);
					  { Fl_Pack* o = m_controlsPack =
						  new Fl_Pack(0, 125, 175, 768-125);
		...
		{ // TODO: remove this extra brace!
			{ Fl_Group* oView = m_viewPane = new Fl_Group(200, 25, 1024-200, 768-25);
				oView->box(FL_NO_BOX);
				oView->color(FL_BACKGROUND_COLOR);
				m_modelerView = new ModelerView(200, 25, 1024-200, 768-25, "");
				w->resizable(m_modelerView);
				oView->end();
				Fl_Group::current()->resizable(oView);
			}
		}
		oWin->end();
	}
